...
function Lazy(source) {
  return _instanceof(source, Iterator) ? source : new Iterator(source);
}
    ...
    if (!iter) {
      collection.each(function (obj) {
        ...
        if (isNil(key) || key < lower || key >= upper) {
          ...
          var _index = findInsertIndex(boundaries, key);

          var boundKey = boundaries[Math.max(0, _index - 1)];
...
function $project(collection, expr, opt) {
  ...
  return collection.map(function (obj) {
    ...
    each(objKeys, function (key) {
      ...
      } else if (inArray([1, true], subExpr)) {// For direct projections, we use the resolved object value
    ...
    value: function stream(collection, query) {
      ...
      if (!isEmpty(this.__operators)) {
        ...
        each(this.__operators, function (operator) {
          ...
          if (query && _instanceof(query, Query)) {
            collection = pipelineOperators[key].call(query, collection, operator[key], _this2.__options);
          } else {
            collection = pipelineOperators[key](collection, operator[key], _this2.__options);
          }
        });
      }
  ...
  _createClass(Query, [{
    ...
    value: function remove(collection) {
      ...
      return reduce(collection, function (acc, obj) {
        if (!_this4.test(obj)) acc.push(obj);
        return acc;
      }, []);
    }
  }]);
